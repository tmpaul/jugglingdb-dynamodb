...
DynamoDB.prototype.define = function (descr) {
  ...
  createTable(this.adapter, tableParams, function (err, data) {
    if (err || !data) {
      console.log(err.toString().red);
      return;
    } else {
  ...
  */
   ...
   */
    ...
    if (breakableValues[outerCounter] === 0) {
      ...
      console.log("Attribute", breakableAttribute, "Datasize: ", dataSize, "bytes", "No. of chunks: ", N);
    } else {
    ...
    async.mapSeries(chunkedData, function (chunked, innerCallback) {
      ...
      chunked[hashKeyAttribute] = data[hashKey];
      ...
      console.log("DYNAMODB >>> SHARDING COMPLETE".blue);
      outerCounter++;
      callback(null, outerCounter);
    }.bind(this));
