  ...
  dynamodb.listTables(function (err, data) {
    ...
    existingTableNames.forEach(function (existingTableName) {
      if (tableParams.TableName === existingTableName) {
        tableExists = true;
        console.log("----------Table %s found in database----------".cyan, existingTableName);
      }
    });
    // If table exists do not create new table
    if (tableExists === false) {
      // DynamoDB will throw error saying table does not exist
      console.log("----------Creating Table: %s in DynamoDB----------".cyan, tableParams.TableName);
      dynamodb.createTable(tableParams, function (err, data) {
        if (err) {
          console.log(err.toString().red);
        } // an error occurred
        else if (!data) {
          console.log("Could not create table".cyan);
        } else {
          console.log("Table created".cyan);
        }; // successful response
      });
    }
  });
...
DynamoDB.prototype.define = function (descr) {
  ...
  // If there are breakable attributes with sharding set to true, create the
  // extra tables now
  ...
  breakableAttributes.forEach(function (breakableAttribute) {
    ...
    shardParams.AttributeDefinitions.push({
        AttributeName: modelName.toLowerCase() + "#" + hashKey,
        AttributeType: helper.TypeLookup(attributeSpecs[hashKey])
    });
    shardParams.KeySchema.push({
    ...
    shardParams.TableName = breakableAttribute;
    ...
    // create shardParams
    createTable(this.adapter, shardParams);
  }.bind(this));
...
DynamoDB.prototype.create = function (model, data, callback) {
  ...
  */
   ...
   */
    ...
    // Set breakableValues
    this._models[model].breakValues[outerCounter] = chunkedData.length;
    // Write chunkedData to database
    for (var innerCounter = 0; innerCounter < chunkedData.length; innerCounter++) {
      chunkParams = {};
      chunkParams.TableName = breakableAttribute;
      var hashKeyAttribute = String(model).toLowerCase() + "#" + hashKey;
      chunkedData[innerCounter][hashKeyAttribute] = data[hashKey];

      var rangeKeyAttribute = String(breakableAttribute) + "#ID";
      chunkedData[innerCounter][rangeKeyAttribute] = innerCounter + 1;

      chunkParams.Item = DynamoFromJSON(chunkedData[innerCounter]);
      this.adapter.putItem(chunkParams, function (err, res){
        if (err) {
          console.log(err.toString().red);
        } else {
          var tempString = "Attribute: `" + breakableAttribute + "` fragment written to database";
          console.log(tempString.cyan);
        }
      });
    }
    outerCounter++;
    ...
    if (tableParams.KeyConditions) {
      ...
      this.adapter.query(tableParams, function (err, res) {
        if (err) {
          ...
          async.mapSeries(res.Items, function (item, callback) {
            var returnData = JSONFromDynamo(item, attributeSpecs);
            helper.GetMyChildrenBack(returnData, model, hashKey, breakables, adapter, callback);
          }, function (err, items) {
            finalResult = items;
