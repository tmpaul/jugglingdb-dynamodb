...
DynamoDB.prototype.attributeSpecs = function (model) {
...
DynamoDB.prototype.create = function (model, data, callback) {
  ...
  */
   ...
   */
    ...
    data = ChunkMe(data, breakableAttribute, N);
    outerCounter++;
  });
        ...
        if (err) {
          ...
          callback(null, null);
        	...
        	res.Items.forEach(function (item){
        		finalResult.push(JSONFromDynamo(item, attributeSpecs));
        	});
  ...
  if (tableParams.KeyConditions) {
    this.adapter.query(tableParams, function (err, res) {
      if (err) {
        ...
        returnData = BuildMeBack(returnData, this._models[model].breakables);
