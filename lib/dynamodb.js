...
DynamoDB.prototype.define = function (descr) {
  ...
  if (descr.settings.tableStatus) {
    var tableStatusWait = descr.settings.tableStatus.waitTillActive;
    if (tableStatusWait === undefined) {
      tableStatusWait = true;
    }
    var timeInterval = descr.settings.tableStatus.timeInterval || 1000;
  } else {
    var tableStatusWait = true;
  ...
  createTable(this.client, tableParams, tableStatusWait, timeInterval, function (err, data) {
    if (err || !data) {
...
logger.log("debug", "Defining model: ",modelName, stopTimer(timeStart).cyan);
};
/**
    ...
    async.mapSeries(chunkedData, function (chunked, innerCallback) {
      ...
      dynamo.putItem(chunkParams, function (err, res) {
        if (err) {
          ...
          var tempString = "PUT FRAGMENT " + String(innerCounter + 1) + " INTO TABLE: " + chunkParams.TableName;
        ...
        if (err) {
          ...
          logger.log("debug", "SHARDING COMPLETE");
          outerCounter++;
      ...
      if (err) {
        ...
        dynamo.putItem(tableParams, function (err, res) {
          if (err) {
            ...
            logger.log("info", queryString, stopTimer(timerStart).cyan);
            if (pKey !== undefined) {
...
function query(model, filter, hashKey, rangeKey, queryString, timeStart) {
  ...
  logger.log("info",queryString, stopTimer(timeStart).cyan);
  return tableParams;
}
        ...
        async.mapSeries(numArray, function (rangeId, innerCallBack) {
          ...
          dynamo.deleteItem(innerChunkParams, function (err, res) {
            if (err) {
              ...
              if (pKey !== undefined) {
                var tempString = "DELETE FRAGMENT FROM TABLE " + chunkParams.TableName + " WHERE " + hashKeyAttribute + " `EQ` " + String(hk) + pkSeparator + String(rk) + " AND " + rangeKeyAttribute + " `EQ` " + rangeId;
              } else {
                var tempString = "DELETE FRAGMENT FROM TABLE " + chunkParams.TableName + " WHERE " + hashKeyAttribute + " `EQ` " + String(hk) + " AND " + rangeKeyAttribute + " `EQ` " + rangeId;
              }
              logger.log("debug",tempString);
    ...
    if (err) {
      ...
      dynamo.deleteItem(tableParams, function (err, res) {
        if (err) {
          ...
          logger.log("info",tempString, stopTimer(timeStart).cyan);
...
DynamoDB.prototype.destroyAll = function(model, callback) {
  ...
  logger.log("info", t.red, stopTimer(timeStart).cyan);
};
