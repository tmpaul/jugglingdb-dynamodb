...
module.exports = {
  ...
  GetMyChildrenBack: function GetMyChildrenBack(data, model, hashKey, breakables, dynamodb, OuterCallback) {
    // Iterate over breakables. Query using data's hashKey
    var hashKeyAttribute = model.toLowerCase() + "#" + hashKey;
    /*
    ...
    async.mapSeries(breakables, function (breakable, callback) {
      ...
      params.KeyConditions[hashKeyAttribute].AttributeValueList.push({
        'S': String(data[hashKey])
      });
      params.TableName = breakable;
      console.log(("PERFORMING QUERY FOR SHARDED ATTRIBUTE: " + breakable).yellow);
