...
DynamoDB.prototype.define = function (descr) {
  ...
  if ((this._models[modelName].rangeKey !== undefined) && (this._models[modelName].pk !== undefined)) {
    if (this._models[modelName].pk !== 'id') {
  ...
  var pk = this._models[modelName].pk;
      ...
      async.mapSeries(breakableAttributes, function (breakableAttribute, callback) {
        ...
        if (pk !== undefined) {
          shardParams.KeySchema.push({
            AttributeName: modelName.toLowerCase() + "#" + pk,
            KeyType: 'HASH'
          });
          logger.log("debug", "HASH KEY:",modelName.toLowerCase() + "#" + pk);
...
DynamoDB.prototype.create = function (model, data, callback) {
  ...
  // If pk is defined, range key is also present.
  if (pk !== undefined) {
    if ((data[rangeKey] === null) || (data[rangeKey] === undefined)) {
      ...
      data[pk] = String(data[hashKey]) + pkSeparator + String(data[rangeKey]);
      originalData[pk] = data[pk];
    }
  }
  ...
  */
   ...
   */
    ...
    async.mapSeries(chunkedData, function (chunked, innerCallback) {
      ...
      if (pk !== undefined) {
        var hashKeyAttribute = String(model).toLowerCase() + "#" + pk;
        chunked[hashKeyAttribute] = String(data[pk]);
      } else {
      ...
      if (err) {
        ...
        dynamo.putItem(tableParams, function (err, res) {
          if (err) {
            ...
            if (pk !== undefined) {
              originalData.id = originalData[pk];
...
DynamoDB.prototype.all = function all(model, filter, callback) {
  ...
  if (hashKeyFound === true) {
    ...
    if (tableParams.KeyConditions) {
      this.client.query(tableParams, function (err, res) {
        if (err) {
          logger.log("error", err.toString());
          callback(err, null);
        } else if (!res) {
          ...
          async.mapSeries(res.Items, function (item, innerCallback) {
            ...
            if ((rangeKey !== undefined) && (pk !== undefined)) {
              returnData[pk] = String(returnData[hashKey]) + pkSeparator + String(returnData[rangeKey]);
              helper.GetMyChildrenBack(returnData, model, pk, breakables, client, innerCallback);
            } else {
    ...
    var attributeSpecs = this._attributeSpecs[model];
    // Scan DynamoDB table
    this.client.scan(tableParams, function (err, res) {
      if (err) {
        logger.log("error", err.toString());
        callback(err, null);
      } else if (!res) {
 ...
 * @param  {Function} callback 
 */
DynamoDB.prototype.find = function find(model, pKey, callback) {
    ...
    async.mapSeries(chunkedData, function (chunked, innerCallback) {
      ...
      dynamo.putItem(chunkParams, function (err, res) {
        if (err) {
          ...
          var tempString = "DYNAMODB >>> PUT FRAGMENT " + String(innerCounter + 1) + " INTO TABLE: " + breakableAttribute;
          logger.log("info", tempString.blue);
          innerCounter++;
          innerCallback(null, breakableAttribute);
        }
   ...
   */
    ...
    async.mapSeries(chunkedData, function (chunked, innerCallback) {
     ...
     if (err) {
      ...
      var attributeSpecs = this._attributeSpecs[model];
      this.client.updateItem(tableParams, function (err, res) {
        if (err) {
          logger.log("error", err.toString());
          callback(err, null);
        } else if (!res) {
...
DynamoDB.prototype.destroy = function (model, pKey, callback) {
        ...
        async.mapSeries(numArray, function (rangeId, innerCallBack) {
          ...
          dynamo.deleteItem(innerChunkParams, function (err, res) {
            if (err) {
              ...
              var tempString = "DYNAMODB >>> DELETE FRAGMENT FROM TABLE " + chunkParams.TableName + " WHERE " + hashKeyAttribute + " `EQ` " + String(hashKey) + " AND " + rangeKeyAttribute + " `EQ` " + rangeId;
              logger.log("info",tempString.blue);
    ...
    if (err) {
      ...
      dynamo.deleteItem(tableParams, function (err, res) {
        if (err) {
          ...
          var tempString = "DYNAMODB >>> DELETE ITEM FROM TABLE " + tableParams.TableName + " WHERE " + hashKey + " `EQ` " + String(hashKey);
          logger.log("info",tempString.blue);
