...
DynamoDB.prototype.create = function (model, data, callback) {
	console.log("DYNAMODB >>> CREATE ITEM %s IN TABLE %s", model, this._tables[model]);
  var tableParams = {};
...
function query(model, filter, hashKey, queryString) {
  ...
  console.log(queryString.magenta);
  return tableParams;
}
...
DynamoDB.prototype.destroy = function(model, hashKey, callback) {
  ...
  tableParams.Key = {};
  // Add hashKey to tableParams
  tableParams.Key[this._models[model].hashKey] = DynamoFromJSON(hashKey)[0];
