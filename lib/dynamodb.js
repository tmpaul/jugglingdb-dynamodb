 ...
 * @param  {object/primitive}   hashKey  [description]
    ...
    this.client.query(tableParams, function (err, res) {
      if (err) {
        ...
        async.mapSeries(res.Items, function (item, callback) {
          var returnData = JSONFromDynamo(item, attributeSpecs);
          helper.GetMyChildrenBack(returnData, model, hashKey, breakables, client, callback);
        }, function (err, items) {
          if (err) {
            logger.log("error", err.toString());
            callback(err, null);
...
DynamoDB.prototype.destroy = function (model, hashKey, callback) {
  ...
  // Add hashKey to tableParams
  tableParams.Key[this._models[model].hashKey] = DynamoFromJSON(hashKey);
  //tableParams.Key[this._models[model].rangeKey] = DynamoFromJSON(rangeKey);
  ...
  var chunkedData = {};
  var dynamo = this.client;
  var parentHashKey = this._models[model].hashKey;
  async.mapSeries(breakableAttributes, function (breakableAttribute, OuterCallback) {
    var chunkParams = {};
    chunkParams.TableName = model + "_" + breakableAttribute;
    var hashKeyAttribute = String(model).toLowerCase() + "#" + parentHashKey;
    var rangeKeyAttribute = String(breakableAttribute) + "#ID";
    ...
    if (err) {
      ...
      dynamo.deleteItem(tableParams, function (err, res) {
        if (err) {
          ...
          var tempString = "DYNAMODB >>> DELETE ITEM FROM TABLE " + tableParams.TableName + " WHERE " + parentHashKey + " `EQ` " + String(hashKey);
          logger.log("info",tempString.blue);
...
/**
 * Remove item deletes an item from the table with given hash key and a range key
 * @param  {[type]}   model    [description]
 * @param  {[type]}   hashKey  [description]
 * @param  {[type]}   rangeKey [description]
 * @param  {Function} callback [description]
 * @return {[type]}            [description]
 */
DynamoDB.prototype.remove = function (model, hashKey, rangeKey, callback) {
  // Use updateItem function of DynamoDB
  var tableParams = {};
  // Set table name as usual
  tableParams.TableName = this.tables(model);
